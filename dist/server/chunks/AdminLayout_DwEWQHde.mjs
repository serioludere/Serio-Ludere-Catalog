...
var $$AdminLayout = createComponent(($$result, $$props, $$slots) => {
	...
	const { title, active, nav = true, meta, mode = "admin" } = Astro.props;
	const shellTab = active ? TAB_TO_SHELL[active] : void 0;
	const heading = title.replace(/^Serio Ludere\s*—\s*/, "");
	return renderTemplate`<html lang="en"${addAttribute(mode, "data-mode")}> <head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, interactive-widget=resizes-content"><meta name="robots" content="noindex, nofollow"><title>${title}</title><link rel="icon" type="image/png" href="/favicon.png"><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400&display=swap" rel="stylesheet">${renderHead($$result)}</head> <body> ${nav ? renderTemplate`${renderComponent($$result, "AppShell", $$AppShell, {
		"active": shellTab,
		"title": heading,
		"meta": meta
	}, {
