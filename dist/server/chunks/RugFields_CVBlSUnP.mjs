...
import { A as driveImageUrl } from "./parse_CyNL3ky6.mjs";
...
var $$RugFields = createComponent(($$result, $$props, $$slots) => {
	...
	const { mode, rug, collections, tags, defaultStatus, roundStep, driveScopeOk, nextId, fetchButton = true, likes = 0 } = Astro.props;
	...
	return renderTemplate` ${maybeRenderHead($$result)}<div class="addform">  <div class="f"> <label for="f_id">Product number</label> <input id="f_id"${addAttribute(rug?.id ?? nextId ?? "", "value")}${addAttribute(edit, "readonly")} autocomplete="off"> <p class="hint">Must be unique — it becomes the product's reference number.</p> </div> <div class="row"> ${!edit && renderTemplate`<div class="f grow"> <label for="yourName">Product name</label> <input id="yourName" placeholder="e.g. Khal Mohammadi" autocomplete="off"> </div>`} <div class="f narrow"> <span class="f__legend">Collections</span> ${renderComponent($$result, "MultiSelect", $$MultiSelect, {
		"id": "f_collection",
		"name": "collection",
		"label": "Collections",
...
Open on site
</a> ` })}`} </div> <div id="m2" class="msg"></div> <input type="hidden" id="f_version"${addAttribute(rug?.version ?? "", "value")}> ${edit && rug && renderTemplate`<p class="hint mono">${metaLine}</p>`} </div> ${edit && renderTemplate`<dialog id="confirm"> <p id="confirmText"></p> <div class="actions">  <button class="btn btn--primary" id="confirmYes" type="button">
Yes
