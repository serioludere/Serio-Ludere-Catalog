import { At as RETAIL_MARKUP, Ct as ADMIN_USER, F as PRODUCT_COLS, H as TABS, I as PRODUCT_HEADER_LABELS, L as PRODUCT_STATUS_CELL, Mt as SITE_URL, N as HEADERS, O as joinCollections, R as PRODUCT_WIDTH, St as ADMIN_SESSION_SECRET, _t as SheetsApiError, gt as SheetContractError, i as getClient, r as getCache, vt as consoleLogger, xt as ADMIN_PASSWORD_HASH, yt as serializeError } from "./runtime_BgX1riZH.mjs";
...
import { c as requestIpHash, f as RateLimiter, i as isSecureSite, l as revalidateState, o as noStore, s as rejectCrossSite, t as ADMIN_MAX_JSON_BODY, u as socketAddressOf } from "./api_B6hDsvkQ.mjs";
import { H as auditRowToCells, R as issuesOf, U as buildAuditRow, V as UnauditableError, c as rowVersion, l as rugVersion, r as adminRugFromCells } from "./read_Cjj8wrWx.mjs";
import { t as invalidateCatalogue } from "./invalidate_DLLO89cK.mjs";
import { n as cellOrClear, t as buildInsertRows } from "./write_nBjqywfQ.mjs";
...
async function ensureProductWidth(client, logger) {
	...
	const requests = [];
	if (columns < PRODUCT_WIDTH) requests.push({ appendDimension: {
		sheetId: sheet.properties.sheetId,
		dimension: "COLUMNS",
		length: PRODUCT_WIDTH - columns
	} });
	const [headerRange] = await client.batchGet([`${TABS.products}!A1:AR1`]);
	const header = headerRange?.values?.[0] ?? [];
	const from = PRODUCT_WIDTH - 2;
	...
	const labels = start < PRODUCT_WIDTH && (named || requests.length) ? PRODUCT_HEADER_LABELS.slice(start).map(String) : [];
...
*/
var GRID_TOO_NARROW = /beyond the last (?:requested|known) column/i;
/**
...
*/
async function commitProductBatch(client, requests, logger) {
	try {
		await client.batchUpdate(requests);
		return;
	} catch (e) {
		if (!(e instanceof SheetsApiError) || !GRID_TOO_NARROW.test(e.message)) onSheetIdError(client, e);
		logger?.error("Products write was wider than the grid; widening and retrying once", { error: serializeError(e) });
	}
	forgetProductWidth();
...
async function readRugRow(client, row) {
	const [vr] = await client.batchGet([`${TABS.products}!A${row}:AR${row}`]);
	return vr?.values?.[0] ?? [];
}
...
async function updateRug(client, args) {
	return withAdminLock(async () => {
		...
		await commitProductBatch(client, buildRugUpdateRequests({
			rugs: await client.sheetIdByTitle(TABS.products),
			auditLog: await client.sheetIdByTitle(TABS.auditLog)
		}, args.row, args.cells, args.audit), args.logger);
...
async function insertRug(client, args) {
	...
	return withAdminLock(async () => {
		...
		await commitProductBatch(client, buildRugInsertRequests({
			rugs: await client.sheetIdByTitle(TABS.products),
			auditLog: await client.sheetIdByTitle(TABS.auditLog)
		}, targetRow, rowCount, args.cells, args.audit), args.logger);
...
async function updateProductCell(client, args) {
	...
	return withAdminLock(async () => {
		...
		const auditSheetId = await client.sheetIdByTitle(TABS.auditLog);
		await commitProductBatch(client, [...args.updates.map((u) => buildRowUpdate(sheetId, u.row, args.columnIndex, [u.value])), ...buildAuditInsert(auditSheetId, args.audit)], args.logger);
		return {
