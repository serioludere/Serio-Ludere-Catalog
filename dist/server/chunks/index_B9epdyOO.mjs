...
import { E as collectionSlugs, T as collectionSlug, i as getClient, mt as driveImageUrl, v as orderedCollectionNames, vt as consoleLogger, yt as serializeError } from "./runtime_BgX1riZH.mjs";
import { a as adminRuntime } from "./http_DfO61_B-.mjs";
import { m as roundStepOf, u as syncLabel, z as SHOPIFY_OPTIONS } from "./read_Cjj8wrWx.mjs";
import { f as dataRot, o as jsonForScript, r as badgesFor } from "./view_BxmrstDZ.mjs";
import { t as dims } from "./units_BFAJ7W-k.mjs";
import { t as $$PageNav } from "./PageNav_Cvw3NuwJ.mjs";
import { n as $$Icon, t as $$AdminLayout } from "./AdminLayout_DYiEsf8i.mjs";
import { t as $$Button } from "./Button_DUWQOLGP.mjs";
import { n as $$Modal, t as $$EmptyState } from "./EmptyState_AkbkU0J2.mjs";
...
var $$RugRow = createComponent(($$result, $$props, $$slots) => {
	...
	return renderTemplate` ${maybeRenderHead($$result)}<div class="irow" role="row" data-card data-row${addAttribute(rug.id, "data-id")}${addAttribute(slug, "data-collection")}${addAttribute(slugs, "data-collections")}${addAttribute(rug.commitStatus === "pending" ? "photos" : "", "data-attention")}${addAttribute(search, "data-search")}> <div class="irow__line" role="presentation"> <span class="irow__id" role="cell">${rug.id}</span> <span class="irow__title" role="cell" data-cell="name">${rug.name}</span> <span class="irow__collection" role="cell">${rug.collection}</span> <span class="irow__size" role="cell">${size}</span> <span class="irow__price" role="cell">${price}</span> <span class="irow__likes" role="cell">${likes > 0 ? likes : "—"}</span>  <span class="irow__shopify" role="cell"> <select${addAttribute(rug.id, "data-shopify")}${addAttribute(`Shopify, product ${rug.id}`, "aria-label")}> <option value=""${addAttribute(rug.shopify === "", "selected")}>—</option> ${SHOPIFY_OPTIONS.map((o) => renderTemplate`<option${addAttribute(o, "value")}${addAttribute(rug.shopify === o, "selected")}> ${o} </option>`)} </select> </span> <span class="irow__source" role="cell"> ${source ? renderTemplate`<a${addAttribute(source.href, "href")} target="_blank" rel="noopener noreferrer"${addAttribute(source.href, "title")}> ${source.label} <span aria-hidden="true"> ↗</span> <span class="sr-only"> (opens in a new tab)</span> </a>` : "—"} </span> <span class="irow__spacer" role="presentation"></span> <span class="irow__action" role="cell"> ${rug.commitStatus === "pending" && renderTemplate`<button type="button" class="chip retry"${addAttribute(rug.id, "data-retry")}>
Finish photo import
...
var $$RugTable = createComponent(($$result, $$props, $$slots) => {
	...
	return renderTemplate`${maybeRenderHead($$result)}<div class="rugtable" role="table" aria-label="Products"> <div class="rugtable__head" role="row"> <span class="irow__id" role="columnheader">ID</span> <span class="irow__title" role="columnheader">Title</span> <span class="irow__collection" role="columnheader">Collection</span> <span class="irow__size" role="columnheader">Size</span> <span class="irow__price" role="columnheader">Price</span> <span class="irow__likes" role="columnheader">Likes</span> <span class="irow__shopify" role="columnheader">Shopify</span> <span class="irow__source" role="columnheader">Source</span>  <span class="irow__spacer" role="presentation"></span> <span class="irow__action" role="columnheader"><span class="sr-only">Actions</span></span> </div> ${rugs.map((rug) => renderTemplate`${renderComponent($$result, "RugRow", $$RugRow, {
		"rug": rug,
