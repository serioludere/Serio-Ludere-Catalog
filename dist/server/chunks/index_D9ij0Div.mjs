...
import { a as $$PreviewFooter, i as $$PreviewHeader, n as $$Reactions, o as $$PreviewControls, r as $$PreviewLayout, t as $$SpecTable } from "./SpecTable_D5uy3vux.mjs";
...
var $$ProductDialog = createComponent(($$result, $$props, $$slots) => {
	...
	return renderTemplate`${cards.map((card) => {
		...
		const priceText = money(card.priceUsd, baseCurrency, rates, "en-US");
		return renderTemplate`<template${addAttribute(card.id, "data-detail")} data-astro-cid-nilyogtc>${templateEnter($$result)} ${maybeRenderHead($$result)}<div class="pv-modal__media" data-astro-cid-nilyogtc> ${photos[0] ? renderTemplate`<img class="pv-modal__photo"${addAttribute(photos[0], "src")}${addAttribute(card.name, "alt")} data-hero-img decoding="async" data-astro-cid-nilyogtc>` : renderTemplate`<span class="pv-modal__empty" data-astro-cid-nilyogtc>Photo to come</span>`} ${photos.length > 1 && renderTemplate`<div class="pv-thumbs pv-modal__thumbs" data-thumbs role="toolbar" aria-label="Photographs" data-astro-cid-nilyogtc> ${photos.map((src, i) => renderTemplate`<button type="button"${addAttribute(["pv-thumb", { "is-on": i === 0 }], "class:list")}${addAttribute(src, "data-full")}${addAttribute(`${card.name}, photograph ${i + 1}`, "data-alt")}${addAttribute(i === 0 ? "true" : void 0, "aria-current")} data-astro-cid-nilyogtc> <img${addAttribute(thumbs[i] ?? src, "src")}${addAttribute(`Photograph ${i + 1}`, "alt")} loading="lazy" decoding="async" data-astro-cid-nilyogtc> </button>`)} </div>`} </div> <div class="pv-modal__info" data-astro-cid-nilyogtc>  <h2 class="pv-h1 pv-modal__name" data-astro-cid-nilyogtc>${card.name}</h2> <p class="pv-price" data-price${addAttribute(card.priceUsd ?? "", "data-usd")}${addAttribute(!priceText, "hidden")} data-astro-cid-nilyogtc> ${priceText} </p> ${card.textureUrl ? renderTemplate`<figure class="pv-modal__texture" data-astro-cid-nilyogtc> <img${addAttribute(card.textureUrl, "src")}${addAttribute(`${card.name}, the weave up close`, "alt")} loading="lazy" decoding="async" data-astro-cid-nilyogtc> <figcaption class="pv-modal__texture-label" data-astro-cid-nilyogtc>texture</figcaption> </figure>` : badgesFor(card.tags.map((t) => t.name)).length > 0 && renderTemplate`<ul class="pv-modal__markers" data-astro-cid-nilyogtc> ${badgesFor(card.tags.map((t) => t.name)).map((b) => renderTemplate`<li class="pv-modal__marker" data-astro-cid-nilyogtc>${b}</li>`)} </ul>`} ${card.description && renderTemplate`<p class="pv-modal__desc" data-astro-cid-nilyogtc>${card.description}</p>`} <div class="pv-modal__react" data-astro-cid-nilyogtc> ${renderComponent($$result, "Reactions", $$Reactions, {
			"rugId": card.id,
			"mode": "detail",
...
var $$CollectionFilters = createComponent(($$result, $$props, $$slots) => {
	...
	return renderTemplate` ${maybeRenderHead($$result)}<nav class="pv-filters" aria-label="Filter by collection" data-astro-cid-hbgmxqev> <button type="button" class="pv-chip is-on" data-filter="all" aria-pressed="true" data-astro-cid-hbgmxqev>All</button> ${collections.map((c) => renderTemplate`<button type="button" class="pv-chip"${addAttribute(c.slug, "data-filter")}${addAttribute(c.description || void 0, "data-description")} aria-pressed="false" data-astro-cid-hbgmxqev> ${c.name} </button>`)} </nav> ${renderScript($$result, "C:/Users/Ramez/Desktop/Serio-Ludere-Catalog/src/components/customer/CollectionFilters.astro?astro&type=script&index=0&lang.ts")}`;
...
var $$Index = createComponent(async ($$result, $$props, $$slots) => {
	...
	return renderTemplate`${renderComponent($$result, "PreviewLayout", $$PreviewLayout, {
		...
		"rates": rates,
		"data-astro-cid-z2nrcivh": true
	})} </div>  <p class="pv-lede pv-collection-intro" data-collection-intro hidden data-astro-cid-z2nrcivh></p> <button class="pv-intro-more" type="button" data-intro-more aria-expanded="false" hidden data-astro-cid-z2nrcivh>
read more
</button> </section> <section class="pv-wrap" data-astro-cid-z2nrcivh> ${cards.length > 0 ? renderTemplate`<div class="pv-grid" data-astro-cid-z2nrcivh> ${cards.map((card, i) => renderTemplate`<div data-card${addAttribute(card.id, "data-rug")}${addAttribute(card.collectionSlugs.join(" "), "data-collections")}${addAttribute(visibleLikes(card.likes), "data-likes")} data-astro-cid-z2nrcivh> ${renderComponent($$result, "ProductCard", $$ProductCard, {
		"card": card,
		"rates": rates,
