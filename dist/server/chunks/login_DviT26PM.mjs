...
import { i as hashPassword, l as verifyPassword } from "./auth_Dq6XU5lm.mjs";
import { c as requestIpHash, o as noStore, s as rejectCrossSite, u as socketAddressOf } from "./api_DdjGbQdl.mjs";
import { u as loadCatalogue } from "./runtime_BIcTruy2.mjs";
import { a as isReservedSlug, n as customerCookieName, o as makeCustomerToken, s as newCustomerSession, t as SLUG_RE } from "./auth_62Lt8qaa.mjs";
import { i as findCustomer, r as customerRuntime } from "./http_CzR8U5Kz.mjs";
...
var DUMMY_HASH = hashPassword("customer-realm-timing-equaliser", {
	N: 2 ** 17,
	r: 8,
	p: 1
});
var POST = async (context) => {
	...
	if (!check.ok) return noStore({
		ok: false,
		error: "too many attempts",
		retryAfterSec: check.retryAfterSec
	}, 429, { "retry-after": String(Math.max(1, check.retryAfterSec)) });
	...
	if (!(verifyPassword(customer?.passwordHash || DUMMY_HASH, parsed.data.password) && Boolean(customer))) {
		const failure = customerRuntime.throttle.fail(ip);
		return noStore({
			ok: false,
			error: "invalid credentials"
		}, 401, { ...failure.retryAfterSec ? { "retry-after": String(failure.retryAfterSec) } : {} });
	}
