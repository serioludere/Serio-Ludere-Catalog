import { C as sizeLabelOf, G as guardedAgent, J as validateOutboundUrl, K as hostnameProblem, S as sizeBandOf, U as JINA_HOST, W as SUPPLIER_HOSTS, X as ScrapeError, Y as SCRAPED_FIELDS, bt as silentLogger, q as isAllowedImageUrl, yt as serializeError } from "./runtime_xH1UDnXO.mjs";
import { r as supplierRetail, t as pricingRuleName } from "./price_Dv81d0Go.mjs";
...
*/
var CM_PAIR_RE = /(\d{2,3}(?:[.,]\d)?)\s*(?:cm\s*)?(?:[x×]|by)\s*(\d{2,3}(?:[.,]\d)?)\s*cm\b/i;
...
*/
var FT_IN_WORDS_PAIR_RE = /(\d{1,2})\s*-?\s*(?:feet|foot|ft)\.?(?:\s*-?\s*(\d{1,2})\s*-?\s*(?:inch(?:es)?|in)\b\.?)?\s*(?:[x×]|by)\s*(\d{1,2})\s*-?\s*(?:feet|foot|ft)\.?(?:\s*-?\s*(\d{1,2})\s*-?\s*(?:inch(?:es)?|in)\b\.?)?/i;
	...
	if (js) try {
		...
		if (isRecord(p) && str(p.title) && str(p.handle)) {
			...
			return {
				...
				vendor: str(p.vendor)?.trim() || void 0,
				productType: str(p.type)?.trim() || void 0,
				available: bool(p.available) ?? (variant ? bool(variant.available) : void 0),
				source: "js"
			};
		}
	} catch {}
	if (json) try {
		...
		if (p && str(p.title) && str(p.handle)) {
			...
			return {
				...
				vendor: str(p.vendor)?.trim() || void 0,
				productType: str(p.product_type)?.trim() || void 0,
				available: variant ? bool(variant.available) : void 0,
				source: "json"
			};
		}
	} catch {}
...
//#endregion
//#region src/lib/scrape/storefront.ts
/** What the form shows when the studio's store lists a rug at 0.00 — its "not priced yet". */
var NO_STORE_PRICE = "the store shows no price for this rug (0.00) — enter the retail price by hand";
/** One `Label: value` line; the label is a few words, the value may be empty ("Method:"). */
var LABEL_RE = /^([A-Za-z][A-Za-z ]{0,30}?)\s*:\s*(.*)$/;
...
function storefrontSpecs(html) {
	...
	for (const line of lines) {
		...
		const key = m?.[1] ? KEYS[m[1].trim().toLowerCase()] : void 0;
...
function titleCase(s) {
	return collapse(s).replace(/(^|[\s-])([a-z])/g, (_, sep, c) => sep + c.toUpperCase());
}
/**
...
*/
function proseFacts(text) {
	const out = {
		colors: [],
		styles: []
	};
	if (!text) return out;
	for (const m of text.matchAll(/\b(?:hand-?made|hand[- ]?knotted|hand[- ]?woven|woven|knotted|made|imported|crafted)\s+(?:in|from)\s+([a-z]+(?:\s[a-z]+)?)/gi)) {
		const lead = [];
	...
	const built = /\bconstructed from\s+(.+?)(?:\s+with\s+[^\n.]*?foundation\b|\s*[.;\n]|$)/i.exec(text) ?? /\b(?:hand[- ]?knotted|hand[- ]?woven|handwoven|woven)(?:\s+flat\s*weave)?\s+from\s+(\d{1,3}%\s*[a-z][a-z ]*?)(?=\s+(?:for|with|on)\b|\s*[.;,\n]|$)/i.exec(text);
...
function parseKaravanSpecs(descriptionHtml) {
	...
	for (let i = 0; i < lines.length - 1; i++) {
		...
		const key = KV_HEADINGS.find((h) => heading === h || heading.startsWith(`${h} `) || heading.startsWith(`${h} &`));
...
* assumed and flagged.
*/
function parseKaravan(handle, src, supplier = "karavanrug") {
	...
	const store = own ? storefrontSpecs(src.html) : {
		colors: [],
		styles: []
	};
	const prose = own ? proseFacts(text) : {
		colors: [],
		styles: []
	};
	const fieldStatus = {};
	...
	if (own && seenPrice !== void 0 && !(seenPrice > 0)) {
		seenPrice = void 0;
	...
	if (size?.note) warnings.push(size.note);
	const stated = (fact, fromProse, field) => {
		if (fact) return fact;
	...
	if (own) {
		const listing = [product.vendor && `vendor ${product.vendor}`, product.productType && `type ${product.productType}`].filter(Boolean);
...
async function scrapeShopifyFirst(det, ctx) {
	...
	if (shouldProbeShopify(det.sourceUrl)) {
		...
		if (!jsOk) {
			if (js && js.status >= 400) lastError = fail(js.status === 403 ? "blocked" : "fetch_failed", `${host} answered HTTP ${js.status} for the .js endpoint`, js.status);
			...
			if (!jsonOk && json && json.status >= 400) return fail(json.status === 403 ? "blocked" : "fetch_failed", json.status === 401 ? `${host} is password-protected (HTTP 401) — a storefront behind Shopify's password page cannot be read until it is open` : `${host} answered HTTP ${json.status}`, json.status);
		}
	}
	let html;
	if (!ctx.signal.aborted && !robotsRefusal(det.htmlUrl, ctx)) try {
		...
		else ctx.logger.warn("product page unavailable; currency assumed", {
			host,
			status: res.status
		});
	} catch (e) {
		...
		ctx.logger.warn("product page fetch failed; currency assumed", {
			host,
			message: err.message
		});
	}
	...
	if (det.supplier === "serioludere" && data.seenPrice !== void 0 && !(data.seenPrice > 0)) {
		data.seenPrice = void 0;
		if (!data.warnings.includes("the store shows no price for this rug (0.00) — enter the retail price by hand")) data.warnings.push(NO_STORE_PRICE);
	}
	if (data.seenPrice === void 0 && det.supplier !== "serioludere") return fail("parse_failed", "no price in the Shopify product payload", void 0, data);
	return {
		ok: true,
		data,
		via
...
async function scrapeRug(input, opts = {}) {
	...
	if ("error" in det) {
		...
		const message = det.error === "unsupported_host" ? "only ecarpetgallery.com, karavanrug.com and serioludere.com product links are supported" : fallback ? "that link has no product in it — open the rug on the supplier site and copy the link from its own page" : "not a supported product link";
		return {
			ok: false,
			code: det.error,
			message,
