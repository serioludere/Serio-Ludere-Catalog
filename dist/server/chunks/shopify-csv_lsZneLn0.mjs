...
import { F as PRODUCT_COLS, H as TABS, I as PRODUCT_HEADER_LABELS, R as PRODUCT_WIDTH, g as assertHeaders, i as getClient } from "./runtime_BgX1riZH.mjs";
...
* are ignored past column AR, and a row with no `Product ID` **and** no `Handle` is a spacer the
...
/**
* `Products!A1:AR` (header row included) as the whole CSV document: BOM, header, one record per
...
var GET = adminGet(async () => {
	const [products] = await getClient().batchGet([`${TABS.products}!A1:AR`]);
