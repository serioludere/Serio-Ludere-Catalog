...
| Sheet                   | Role                                                                                                                                                      | Reality                                                                               |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `src/styles/tokens.css` | 8 palette values from `reference/catalogue.html`, marked _"Do not edit values"_ (`src/styles/tokens.css:1`), plus ~25 additive tokens from the Folio pass | **values largely shadowed** by `modes.css`, which loads after it in all three layouts |
...
why: _"success and danger are 3.18:1 on surface today, which is why nothing should set
data-mode=admin-dark until they exist"_ (`src/styles/modes.css:405-409`). Dark mode is blocked on a
...
is not "no work" — it constrains _how_, not _whether_.
...
- **CSP delivery — answered, and the answer is worse than either option.** An earlier revision of this
  ...
  (`node_modules/astro/dist/types/public/config.d.ts:759`). That prose describes the _prerendered_ case only.
