// Security headers on every response (docs/ADR.md D1), then the root redirect, then the admin gate
// (docs/ADMIN_SPEC.md §2.2), then the customer realm gate (brief §10). Cached responses keep the
// headers they were rendered with, so cache hits carry them too; admin and customer responses are
// never cached at all.
//
// There are exactly two login pages: /admin/login and each buyer's own /{slug}. The site-password
// page that used to sit in front of `/` is gone (owner, 2026-09-17) — the bare domain is the studio's
// front door to the admin, not a third screen asking for a third password.
//
 ...
 */
const root = defineMiddleware((context, next) => {
...
export const onRequest = sequence(securityHeaders, root, admin, customer);
